...
```
/                          → Cities directory (global) — every publicly live city, each with a small map
...
- Required categories are derived from the distinct `category` values of **live** entries in `data/services.json` (currently `roadside`, `windshield`, and `carb`).
...
1. Open `data/services.json` and copy an existing entry in the right `category` (`"roadside"`, `"windshield"`, or `"carb"` — or add a new category by giving it a new `id` in `data/site.json`'s `offers[]` first, with its own `categoryName`).
...
Maps use the free, **keyless** Google Maps embed — a plain iframe pointed at `https://www.google.com/maps?q={city}&output=embed`. No API key, no Google Cloud Console project, no billing account, and nothing to configure — it just works out of the box for any city you add. This is deliberate: the alternative (the Maps JavaScript API) requires a billing-enabled Google Cloud project and a domain-restricted key before anything renders, which is unnecessary complexity for a simple "here's where we are" map.

- The global Cities directory (`/`) shows a small embed on every city's card.
- Each city's `/​{slug}/locations/` page shows a larger embed of that one city, next to its area list.

Both are built from `city.cityName`/`city.stateAbbr` in `data/cities.json` — nothing to set up when you add a new city, the map just works.

**Trade-off worth knowing:** this keyless embed is pan/zoom only — no custom markers, no dark map styling, no coverage-radius circle drawn on the map itself (the radius is stated in the text next to it instead). If you ever need custom markers, dark-themed map styling, or drawn coverage circles, that requires switching back to the Maps JavaScript API with a real, domain-restricted API key — a bigger, deliberate trade-off, not something to reach for by default.
